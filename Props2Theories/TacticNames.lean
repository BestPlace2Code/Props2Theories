...
elab "elim_andc" hlr:term "," hl:ident "," hr:ident : tactic => do
  evalTactic (← `(tactic|
    have $hl:ident := And.left $hlr;
    have $hr:ident := And.right $hlr;
    clear $hlr
...
elab "_apply_lc" hlir:term "," hl:term "," hr:ident : tactic => do
  evalTactic (← `(tactic|
    have $hr := (Iff.mp $hlir) ($hl);
    clear $hl
  ))

elab "_apply_rc" hlir:term "," hr:term "," hl:ident : tactic => do
  evalTactic (← `(tactic|
    have $hl := (Iff.mpr $hlir) ($hr);
    clear $hr
  ))
...
elab "by_contra_cl" h_np:ident : tactic => do
  evalTactic (← `(tactic|
      apply dneg_elim;
      intro $h_np:ident;
...
elab "_by_contra_cl" h_npF:term "," h_p:ident : tactic => do
  evalTactic (← `(tactic|
...
elab "elim_existsc" h:term "," a:ident "," ha:ident : tactic => do
  evalTactic (← `(tactic|
     let ⟨$a, $ha⟩ := $h;
     clear $h
  ))
...
elab "propext_" : tactic => do
  evalTactic (← `(tactic|
    apply propext;
    assumption
  ))

elab "_propext" h:term "," h_new:ident : tactic => do
  evalTactic (← `(tactic|
    have $h_new : _ = _ := propext $h
  ))


elab "_propext_" : tactic => do
  evalTactic (← `(tactic|
    apply propext
  ))
...
elab "funext" h:term : tactic => do
  evalTactic (← `(tactic|
    funext _x;
    apply $h
  ))

elab "funext_" : tactic => do
  evalTactic (← `(tactic|
    apply funext;
    assumption
  ))

elab "_funext_" : tactic => do
  evalTactic (← `(tactic|
    apply funext
  ))


elab "_funext" h:term "," h_new:ident : tactic => do
  evalTactic (← `(tactic|
...
macro "elim_exists_uniquec" h:term "," a:ident "," hpa:ident "," hfora:ident : tactic =>
  `(tactic|
      (
        let ⟨$a, ⟨$hpa, $hfora⟩⟩ := $h
        try dsimp (config := { beta := true, zeta := false, iota := false }) at $hpa $hfora;
